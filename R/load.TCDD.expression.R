...
load.TCDD.expression <- function(datasets.to.load = 'all', verbose = FALSE) {
	...
	# load all requested datasets one-by-one
        for (dataset in datasets.to.load) {
                ...
                # read expression data from disk
                expression.data <- read.delim(
                        paste(
				dataset.directory, 
				datasets$Expression.file[datasets$Dataset == dataset], 
				sep = ''
				),
                        header = TRUE,
                        sep = '\t'
		...
		# read phenodata from disk
		phenodata.file <- read.delim(
			paste(
				dataset.directory,
				datasets$Phenodata.file[datasets$Dataset == dataset],
				sep = ''
				),
			header = TRUE,
			sep = '\t'
                ...
                # save final objects
                expression[[dataset]] <- expression.data;
		phenodata[[dataset]]  <- phenodata.file;
		}
